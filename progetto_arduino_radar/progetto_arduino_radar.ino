...
unsigned long Millisecondi_servo = 0;                               //variabile che misura i millisecondi per il conteggio del servo
unsigned long intervallo_servo = 0;                                 //variabile che misura l'intervallo di tempo del servo
...
void stampa_distanza(){
  unsigned long tempo_attuale_LCD = millis();                       //intervallo di tempo del motore
...
void motoreAutomatico() {
  unsigned long millisecondi_servo = millis();                      //crea la variabile che registra il tempo relativo al servomotore
  if(millisecondi_servo - intervallo_servo >= 15){                  //se il tempo trascorso meno i millisecondi impostati superano l'intervallo, allora aggiorna la posizione del motore
  ...
  stampa_distanza();                                                //stampa a video la distanza ogni aggiornamento di posizione
  }
}
...
void controllaMotore() {
  ...
  stampa_distanza();                                                //stampa a video la distanza ogni aggiornamento di posizion
}
...
void loop() {
  ...
  if(lettura == LOW && (millis() - Millisecondi_servo > ritardo)){  //se rileva un cambio di stato e registra un ritardo maggiore 
    stato = !stato;                                                 //allora inverti lo stato
    Millisecondi_servo = millis();                                  //e reimposta i millisecondi
  }
